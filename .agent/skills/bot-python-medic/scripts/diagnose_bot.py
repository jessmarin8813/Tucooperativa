    ...
    if import_match:
        ...
        for usage in set(usages):
            if usage in required_ext and usage not in imported_names:
                errors.append(f"[WARN] ERROR DE IMPORTACIÓN: '{usage}' se usa pero no está en la lista de imports")

    if 'handle_callback' in content and 'CallbackQueryHandler(handle_callback)' not in content:
        errors.append("[WARN] REGISTRO FALTANTE: Se definió 'handle_callback' pero no está registrado.")

    if "add_error_handler" not in content:
        errors.append("[WARN] ESTABILIDAD: No se ha registrado un 'error_handler' global.")

    # --- 2. Auditoría de Conectividad (PRE-FLIGHT) ---
    if os.path.exists(client_path) and base_url:
        print(f"[AUDIT] Auditando Conectividad API en {base_url}...")
        ...
        for endpoint in set(endpoints):
            ...
            try:
                ...
                if res.status_code == 404:
                    errors.append(f"[CRITICAL] ENDPOINT ROTO (404): No existe '{endpoint}.php' en el backend. Revisar rutas en api_client.py")
                elif res.status_code >= 500:
                    errors.append(f"[WARN] ERROR SERVIDOR (500): '{endpoint}.php' está fallando en el backend.")
            except Exception as e:
                # Si falla la conexión, avisamos pero no matamos el build (podría ser red temporal)
                print(f"[WARN] No se pudo verificar {endpoint}.php ({e})")

    # --- Resultado Final ---
    if not errors:
        print("[OK] Bot Médico: No se detectaron anomalías estructurales ni de conectividad.")
        sys.exit(0)
    else:
        print("[CRITICAL] Bot Médico detectó problemas:")
        for err in errors:
            print(err)
        sys.exit(1)
