...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$chofer) {
            sendResponse(['error' => 'Chofer no vinculado (ID: '.$tid.')'], 401);
        }
        ...
        if (!empty($vehicle['owner_tid'])) {
            $msg = "📢 *REPORTE DE INCIDENCIA*\n" .
                   ...
                   "📟 ODO: `{$odo_val}` km\n\n" .
                   "🛠️ *UNIDAD BLOQUEADA*\n" .
                   "La unidad ha entrado en mantenimiento. El chofer debe reportar reparación para reactivar.";
        ...
        sendResponse([
            'success' => true, 
            'message' => 'Falla reportada y unidad bloqueada.',
            'vehiculo_placa' => $vehicle['placa'],
            'ruta_activa' => ($ruta_id !== null)
        ]);
    }
} catch (Exception $e) {
